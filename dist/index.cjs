...
function log(level, message) {
  ...
  const coloredMsg = level === "error" ? import_chalk2.default.red(message) : level === "warn" ? import_chalk2.default.yellow(message) : level === "success" ? import_chalk2.default.green(message) : level === "debug" ? import_chalk2.default.dim(message) : message;
  console.log(`${prefix} ${coloredMsg}`);
}
var import_chalk2, PREFIXES, verbose;
var init_logger = __esm({
  "src/utils/logger.ts"() {
    "use strict";
    import_chalk2 = __toESM(require("chalk"), 1);
    PREFIXES = {
      debug: import_chalk2.default.dim("  \u283F"),
      info: import_chalk2.default.blue("  \u2139"),
...
__export(index_exports, {
  ...
  DocWalkConfigSchema: () => DocWalkConfigSchema,
  FREE_PRESET_IDS: () => FREE_PRESET_IDS,
  PREMIUM_PRESET_IDS: () => PREMIUM_PRESET_IDS,
  ...
  getFreePresetIds: () => getFreePresetIds,
  getLanguageDisplayName: () => getLanguageDisplayName,
  getParser: () => getParser,
  getPremiumPresetIds: () => getPremiumPresetIds,
  getPresetIds: () => getPresetIds,
  getProvider: () => getProvider,
  getRegisteredLanguages: () => getRegisteredLanguages,
  getSupportedLanguages: () => getSupportedLanguages,
  isPremiumPreset: () => isPremiumPreset,
  loadConfig: () => loadConfig,
  loadConfigFile: () => loadConfigFile,
  loadPremiumPresets: () => loadPremiumPresets,
...
var DomainSchema = import_zod.z.object({
...
var VersioningSchema = import_zod.z.object({
...
var DocWalkConfigSchema = import_zod.z.object({
...
var explorer = (0, import_cosmiconfig.cosmiconfig)(MODULE_NAME, {
  searchPlaces: [
    "docwalk.config.yml",
    "docwalk.config.yaml",
    "docwalk.config.json",
...
async function loadConfig(searchFrom) {
  ...
  if (!result || result.isEmpty) {
    throw new ConfigNotFoundError(
      `No docwalk configuration found. Run ${import_chalk.default.cyan("docwalk init")} to create one.`
    );
  }
...
async function loadConfigFile(filepath) {
  ...
  if (!result || result.isEmpty) {
    throw new ConfigNotFoundError(
      `Configuration file is empty: ${import_chalk.default.dim(filepath)}`
    );
  }
  const parsed = DocWalkConfigSchema.safeParse(result.config);
  if (!parsed.success) {
...
async function initParser(language) {
  ...
  if (!languageCache.has(language)) {
    ...
    const wasmDir = import_path.default.dirname(require2.resolve("tree-sitter-wasms/package.json"));
    const wasmPath = import_path.default.join(wasmDir, "out", wasmFile);
...
var PhpParser = class {
  ...
  async parse(content, filePath) {
    ...
    this.walkNode(program, filePath, content, symbols, imports, exports2);
    return { symbols, imports, exports: exports2, moduleDoc };
  }
  walkNode(node, filePath, content, symbols, imports, exports2) {
    for (let i = 0; i < node.childCount; i++) {
...
async function detectWorkspaces(repoRoot) {
  try {
    const pkgJsonPath = import_path2.default.join(repoRoot, "package.json");
    const pkgJson = JSON.parse(await (0, import_promises.readFile)(pkgJsonPath, "utf-8"));
  ...
  try {
    const pnpmPath = import_path2.default.join(repoRoot, "pnpm-workspace.yaml");
    const content = await (0, import_promises.readFile)(pnpmPath, "utf-8");
  ...
  try {
    const lernaPath = import_path2.default.join(repoRoot, "lerna.json");
    const lernaJson = JSON.parse(await (0, import_promises.readFile)(lernaPath, "utf-8"));
...
async function resolveWorkspaceGlobs(repoRoot, globs) {
  ...
  for (const match of matches) {
    try {
      const fullPath = import_path2.default.join(repoRoot, match);
      const pkgJson = JSON.parse(await (0, import_promises.readFile)(fullPath, "utf-8"));
      const name = pkgJson.name;
      if (name) {
        const dir = import_path2.default.dirname(match);
...
async function analyzeCodebase(options) {
  ...
  if (files.length === 0) {
    ...
    try {
      ...
      for (const f of allFiles) {
        const ext = import_path3.default.extname(f) || "(no extension)";
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const absolutePath = import_path3.default.resolve(repoRoot, filePath);
    onProgress?.(i + 1, files.length, filePath);
    try {
      const fileStat = await (0, import_promises2.stat)(absolutePath);
...
function resolveImportSource(source, fromFile, knownFiles, workspaceInfo) {
  if (source.startsWith(".") || source.startsWith("@/")) {
    ...
    let resolved = source.startsWith("@/") ? source.replace("@/", "src/") : import_path3.default.join(dir, source);
    ...
    ].map((c) => import_path3.default.normalize(c));
    return knownFiles.find((f) => candidates.includes(import_path3.default.normalize(f)));
  }
    ...
    for (const [pkgName, pkgDir] of workspaceInfo.packages) {
      if (source === pkgName || source.startsWith(`${pkgName}/`)) {
        ...
        const match = knownFiles.find(
          (f) => candidates.some((c) => import_path3.default.normalize(f) === import_path3.default.normalize(c))
        );
...
function computeProjectMeta(modules, repoRoot, source) {
  ...
  if (rawName === ".") {
    name = pkgData?.name || import_path3.default.basename(repoRoot);
  } else {
    name = rawName;
  }
...
async function runSync(options) {
  ...
  const statePath = import_path4.default.resolve(repoRoot, sync.state_file);
  const manifestPath = import_path4.default.resolve(repoRoot, ".docwalk/manifest.json");
...
async function isManifestValid(manifestPath) {
  try {
    await (0, import_promises3.access)(manifestPath);
...
async function loadSyncState(statePath) {
  try {
    const content = await (0, import_promises3.readFile)(statePath, "utf-8");
    const parsed = JSON.parse(content);
...
async function saveSyncState(statePath, state) {
  await (0, import_promises3.mkdir)(import_path4.default.dirname(statePath), { recursive: true });
  await (0, import_promises3.writeFile)(statePath, JSON.stringify(state, null, 2));
}
...
async function saveManifest(manifestPath, manifest) {
  await (0, import_promises3.mkdir)(import_path4.default.dirname(manifestPath), { recursive: true });
  await (0, import_promises3.writeFile)(manifestPath, JSON.stringify(manifest, null, 2));
}
