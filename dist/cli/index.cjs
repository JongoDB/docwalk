...
async function loadConfig(searchFrom) {
  ...
  if (!parsed.success) {
    const errors = parsed.error.issues.map((issue) => {
      ...
      return `  ${import_chalk3.default.red("\u2717")} ${import_chalk3.default.dim(path28)}: ${issue.message}`;
    }).join("\n");
...
function detectCircularDependencies(manifest) {
  ...
  const cycles = [];
  function dfs(node, path28) {
    if (inStack.has(node)) {
      const cycleStart = path28.indexOf(node);
      if (cycleStart !== -1) {
        cycles.push(path28.slice(cycleStart));
      }
      return;
    }
...
var import_promises3, import_fs, import_path4, import_fast_glob3;
...
var import_path15, import_simple_git;
...
`;
  ...
  if (config.analysis.qa_widget) {
    ...
    try {
      ...
      qaWidgetAssets = await injectQAWidget2(outputDir, config.analysis.qa_config);
    } catch (err) {
...
function generateMkdocsConfig(manifest, config, navigation, qaWidgetAssets) {
...
var init_mkdocs = __esm({
  "src/generators/mkdocs.ts"() {
    ...
    init_architecture();
    init_providers();
    MERMAID_ZOOM_JS = `/* DocWalk \u2014 Mermaid rendering + click-to-zoom */
(function() {
  var diagrams = [];
...
async function generateCommand(options) {
  ...
  if (options.depth) {
    ...
    log("info", `Documentation depth: ${import_chalk4.default.bold(options.depth)}`);
  }
  if (options.ai) {
  ...
  if (isInteractive && !hasAnyFlag) {
    ...
    blank();
    const { theme: chosenTheme } = await import_inquirer2.default.prompt([
      {
        type: "list",
        name: "theme",
    ...
    const { layout: chosenLayout } = await import_inquirer2.default.prompt([
      {
        type: "list",
        name: "layout",
    ...
    const { depth: chosenDepth } = await import_inquirer2.default.prompt([
      {
        type: "list",
        name: "depth",
    ...
    if (!config.analysis.ai_summaries) {
      ...
      if (hasKey) {
        log("info", `AI: ${import_chalk4.default.green("enabled")} (${prov.name})`);
      } else {
  ...
  step = totalSteps;
  const outputDir = import_path18.default.resolve(options.output);
...
async function writeConfigAndScaffold(config) {
  const configPath = import_path19.default.resolve("docwalk.config.yml");
...
async function runSync(options) {
  ...
  const statePath = import_path20.default.resolve(repoRoot, sync.state_file);
  const manifestPath = import_path20.default.resolve(repoRoot, ".docwalk/manifest.json");
...
async function deployCommand(options) {
  ...
  const buildDir = import_path23.default.resolve(deployConfig.output_dir || "site");
...
var import_chalk8, import_path23;
...
__export(doctor_exports, {
  ...
  hasVenv: () => hasVenv,
  hasZensicalInVenv: () => hasZensicalInVenv,
  installZensical: () => installZensical,
  zensicalBin: () => zensicalBin
});
function venvPython() {
  return import_path24.default.join(VENV_DIR, "bin", "python3");
}
function venvPip() {
  return import_path24.default.join(VENV_DIR, "bin", "pip");
}
function zensicalBin() {
  return import_path24.default.join(VENV_DIR, "bin", "zensical");
}
function hasVenv() {
  return (0, import_fs2.existsSync)(venvPython());
}
function hasZensicalInVenv() {
  return (0, import_fs2.existsSync)(zensicalBin());
}
...
async function doctorCommand(options) {
  ...
  console.log(import_chalk10.default.bold("  Prerequisites"));
  console.log(import_chalk10.default.dim("  \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"));
  for (const result of results) {
    if (result.ok) {
      const version = result.version ? import_chalk10.default.dim(` ${result.version}`) : "";
      const detail = result.detail ? import_chalk10.default.dim(` (${result.detail})`) : "";
      console.log(`  ${import_chalk10.default.green("\u2713")} ${result.name}${version}${detail}`);
    } else {
      const detail = result.detail ? import_chalk10.default.dim(` \u2014 ${result.detail}`) : "";
      console.log(`  ${import_chalk10.default.red("\u2717")} ${result.name}${detail}`);
    }
  }
  blank();
  ...
  if (options.install) {
    ...
    log("info", `Run ${import_chalk10.default.cyan("docwalk doctor --install")} to install Zensical.`);
  }
}
async function installZensical() {
  const { execa } = await import("execa");
  const { mkdir: mkdir8 } = await import("fs/promises");
  await mkdir8(".docwalk", { recursive: true });
  if (!hasVenv()) {
  ...
  try {
    ...
    blank();
    log("success", `Zensical installed to ${import_chalk10.default.dim(".docwalk/venv/")}`);
    return true;
  } catch (err) {
    blank();
...
var import_chalk10, import_path24, import_fs2, VENV_DIR;
var init_doctor = __esm({
  "src/cli/commands/doctor.ts"() {
    ...
    init_cli_tools();
    init_logger();
    VENV_DIR = import_path24.default.resolve(".docwalk", "venv");
  }
});
...
async function startWatcher(repoRoot, include, verbose2) {
  ...
  for (const pattern of include) {
    ...
    if (topDir) {
      const fullDir = import_path25.default.resolve(repoRoot, topDir);
      try {
  ...
  for (const dir of watchDirs) {
    log("debug", `Watching ${import_chalk11.default.dim(dir)}`);
...
async function findZensical() {
  const { hasZensicalInVenv: hasZensicalInVenv2, zensicalBin: zensicalBin2 } = await Promise.resolve().then(() => (init_doctor(), doctor_exports));
  if (hasZensicalInVenv2()) {
    return zensicalBin2();
  }
  try {
    const { execa } = await import("execa");
...
async function devCommand(options) {
  ...
  const outputDir = import_path25.default.resolve("docwalk-output");
  const fs = await import("fs");
  if (!fs.existsSync(import_path25.default.join(outputDir, "docs"))) {
  ...
  if (!zensical) {
    ...
    blank();
    const { installZensical: installZensical2, zensicalBin: zensicalBin2 } = await Promise.resolve().then(() => (init_doctor(), doctor_exports));
    const installed = await installZensical2();
    if (installed) {
      zensical = zensicalBin2();
    } else {
      log("error", "Could not install Zensical. Please install Python 3 first:");
      console.log(`    ${import_chalk11.default.cyan("brew install python")}     ${import_chalk11.default.dim("# macOS")}`);
      console.log(`    ${import_chalk11.default.cyan("sudo apt install python3")} ${import_chalk11.default.dim("# Ubuntu/Debian")}`);
      process.exit(1);
    }
    blank();
  }
...
var import_chalk11, import_path25;
var init_dev = __esm({
  "src/cli/commands/dev.ts"() {
    "use strict";
...
async function statusCommand(options) {
  ...
  try {
    const { config, filepath } = options.config ? await loadConfigFile(options.config) : await loadConfig();
    console.log(import_chalk12.default.dim("  Configuration"));
    console.log(`    Config file:    ${import_chalk12.default.cyan(filepath)}`);
    console.log(`    Repository:     ${import_chalk12.default.cyan(config.source.repo)}`);
    console.log(`    Branch:         ${config.source.branch}`);
    console.log(`    Provider:       ${config.deploy.provider}`);
    console.log(`    Domain:         ${config.domain.custom || import_chalk12.default.dim("(none)")}`);
    ...
    console.log(`    AI summaries:   ${config.analysis.ai_summaries ? import_chalk12.default.green("enabled") : import_chalk12.default.dim("disabled")}`);
    blank();
    ...
    try {
      ...
      console.log(import_chalk12.default.dim("  Sync State"));
      console.log(`    ${import_chalk12.default.yellow("Not synced yet")} \u2014 run ${import_chalk12.default.cyan("docwalk generate")} first`);
      blank();
    }
  } catch (error) {
