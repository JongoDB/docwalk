...
// ── Core Mermaid renderer + click-to-zoom (injected for all presets) ────
// Mermaid's render() returns sanitized SVG — safe for DOM insertion.
...
export async function generateDocs(options: GenerateOptions): Promise<void> {
  ...
  // ── 3b. Write core Mermaid renderer + zoom (all presets) ──────────────
  {
    const jsDir = path.join(docsDir, "javascripts");
    await mkdir(jsDir, { recursive: true });
...
function generateMkdocsConfig(
  ...
  const extraJs: string[] = ["javascripts/mermaid-zoom.js"];
  if (preset?.customJs) {
